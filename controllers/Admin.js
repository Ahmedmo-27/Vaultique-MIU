...
exports.renderAnalytics = async (req, res) => {
    try {
        ...
        const currentBounceRate = bounceRates[0] ? (bounceRates[0].bounced / bounceRates[0].total) * 100 : 0;
        const previousBounceRate = bounceRates[1] ? (bounceRates[1].bounced / bounceRates[1].total) * 100 : 0;
        ...
        const sessionDurationChange = previousDuration ? ((currentDuration - previousDuration) / previousDuration) * 100 : 0;
        ...
        res.render('analytics', {
            ...
            totalUsers,
            totalSessions,
            userGrowth: userGrowth.length > 1 ? ((userGrowth[0].count - userGrowth[1].count) / userGrowth[1].count) * 100 : 0,
            sessionGrowth: sessionGrowth.length > 1 ? ((sessionGrowth[0].count - sessionGrowth[1].count) / sessionGrowth[1].count) * 100 : 0,
