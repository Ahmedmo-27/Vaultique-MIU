...
function updateCollectionPage(collectionData) {
  ...
  if (collectionHeader)
    collectionHeader.textContent = collectionData.header || 'Premium Collection';
  if (collectionDescription)
    collectionDescription.textContent =
      collectionData.description || 'Discover our exquisite collection of timepieces.';
...
function updateBrandPage(brandData) {
  ...
  if (brandDescription)
    brandDescription.textContent =
      brandData.description || 'Discover our exquisite collection of timepieces.';
...
function setupFilterEventListeners() {
  ...
  document
    ...
    ?.addEventListener('click', () => handlePageChange(state.currentPage - 1));
  document
...
function handleFilterChange(event) {
  ...
  document.querySelectorAll('.filters select').forEach((select) => {
    const isEntityFilter =
      (state.pageType === 'collection' && select.id === 'collection') ||
...
function clearFilters() {
  ...
  const entitySelect = document.getElementById(
    state.pageType === 'collection' ? 'collection' : 'brand'
  );
  ...
  if (currentEntityValue) {
    state.currentFilters[state.pageType === 'collection' ? 'collection' : 'brand'] =
      currentEntityValue;
  }
...
async function initializePage() {
  ...
  try {
    ...
    const entityName =
      state.pageType === 'collection' ? extractCollectionFromURL() : extractBrandFromURL();
    ...
    console.group(
      `${state.pageType.charAt(0).toUpperCase() + state.pageType.slice(1)} Page Loading`
    );
    ...
    if (!response.ok) {
      ...
      createModal('Error', errorContent, [
        {
          text: 'Retry',
          class: 'confirm-btn',
          clickHandler: () => {
    ...
    const featuredItems =
      state.pageType === 'collection' ? entityData.featuredItems : entityData.featuredModels;
