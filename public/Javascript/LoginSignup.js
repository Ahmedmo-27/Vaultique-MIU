document.addEventListener('DOMContentLoaded', function () {
  ...
  // Clean up any stored credentials on page load
  // This helps prevent issues with hashed passwords being reused
  const cleanupStoredCredentials = () => {
    // Check for any stored passwords or hashes that might be causing issues
    if (localStorage.getItem('password') || 
        localStorage.getItem('hashedPassword') ||
        localStorage.getItem('credentials')) {
      console.log('Cleaning up stored credentials');
      localStorage.removeItem('password');
      localStorage.removeItem('hashedPassword');
      localStorage.removeItem('credentials');
    }
    
    // Ensure password fields are clear
  ...
  signupForm.addEventListener('submit', async function (e) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Store JWT token if provided in the response
